...
class PhotoQuestionBloc extends Bloc<PhotoQuestionEvent, PhotoQuestionState> {
  ...
  PhotoQuestionBloc({required this.repository}) : super(const PhotoQuestionState()) {
  ...
  void onEventSelectAnswer(SelectAnswer event, Emitter<PhotoQuestionState> emit) {
    if (event.answer == 1) {
      if (_questionIndex < 3) {
        debugPrint("3 ballik");
        repository.photoQuestionResult += 3;
      } else if (_questionIndex == 8 || _questionIndex == 9 || _questionIndex == 5 || _questionIndex == 6) {
